...
Hekas uses [Tendermint’s coding repo](https://github.com/tendermint/coding)
...
3. If you would like to contribute:
    ...
    8. Please make sure to run `make format` before every commit -
       ...
       There are CI tests built into the Hekas repository
...
Check out our [good first issues](https://github.com/hekas-network/hekas/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Hekas,
please create an [ADR](https://github.com/hekas-network/hekas/blob/main/docs/architecture/README.md)
...
While my fork lives at `https://github.com/hekas-network/hekas`,
the code should never exist at `$GOPATH/src/github.com/hekas-network/hekas`.
Instead, we use `git remote` to add the fork as a new remote for the original repo,`$GOPATH/src/github.com/hekas-network/hekas`,
...
Now `origin` refers to my fork and `upstream` refers to the hekas version.
So I can `git push -u origin master` to update my fork,
and make pull requests to hekas from there.
...
**Note**: tharsis’s Hekas team currently cuts releases on a need to have basis.
