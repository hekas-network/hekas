...
HTTPS_GIT := https://github.com/hekas-network/hekas.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=hekas \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(HEKAS_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm hekasd || true
	# create a new container from the latest image
	$(DOCKER) create --name hekas -t -i ${DOCKER_IMAGE}:latest hekas
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp hekas:/usr/bin/hekasd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/hekas-network/hekas"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(HEKAS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/hekas:Z hekas/node "./hekasd testnet init-files --v 4 -o /hekas --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\hekasd:/hekas\Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)\build\node1\hekasd:/hekas\Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)\build\node2\hekasd:/hekas\Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)\build\node3\hekasd:/hekas\Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
else
	@docker run --rm -v $(CURDIR)/build/node0/hekasd:/hekas:Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)/build/node1/hekasd:/hekas:Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)/build/node2/hekasd:/hekas:Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
	@docker run --rm -v $(CURDIR)/build/node3/hekasd:/hekas:Z hekas/node "./hekasd tendermint unsafe-reset-all --home=/hekas"
endif
...
PACKAGE_NAME:=github.com/hekas-network/hekas
