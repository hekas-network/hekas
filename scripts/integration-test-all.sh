...
## default port prefixes for hekasd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/hekasd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/hekasd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/hekasd add-genesis-account \
    "$("$PWD"/build/hekasd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000ahekas,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/hekasd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/hekasd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/hekasd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/hekasd start --pruning=nothing --rpc.unsafe \
    ...
    HEKAS_PID=$!
    echo "started hekas node, pid=$HEKAS_PID"
    # add PID to array
    arr+=("$HEKAS_PID")
...
if [[ -z $TEST || $TEST == "rpc" ||  $TEST == "pending" ]]; then
    ...
    for i in $(seq 1 "$TEST_QTD"); do
        ...
        echo "going to test hekas node $HOST_RPC ..."
...
stop_func() {
    HEKAS_PID=$i
    echo "shutting down node, pid=$HEKAS_PID ..."
