...
import (
	...
	"github.com/hekas-network/hekas/x/inflation/types"
)
...
func (suite *KeeperTestSuite) TestMintAndAllocateInflation() {
	testCases := []struct {
	...
	}{
		{
			...
			func() {},
			sdk.NewCoin(denomMint, sdk.NewInt(716_666)),
			sdk.NewCoin(denomMint, sdk.NewInt(150_000)),
...
func (suite *KeeperTestSuite) TestGetCirculatingSupplyAndInflationRate() {
	...
	bondedCoins := sdk.NewDecCoin(hekastypes.AttoHekas, bondedAmt)
	...
	testCases := []struct {
	...
	}{
		{
			...
			sdk.TokensFromConsensusPower(400_000_000, hekastypes.PowerReduction).Sub(bondedAmt),
			func() {
		...
		{
			"high supply",
			sdk.TokensFromConsensusPower(800_000_000, hekastypes.PowerReduction).Sub(bondedAmt),
			func() {},
			sdk.MustNewDecFromStr("1.073333333333333300"),
		},
		{
			"low supply",
			sdk.TokensFromConsensusPower(400_000_000, hekastypes.PowerReduction).Sub(bondedAmt),
			func() {},
			sdk.MustNewDecFromStr("3.220000000000000000"),
		},
		{
			"zero circulating supply",
			sdk.TokensFromConsensusPower(200_000_000, hekastypes.PowerReduction).Sub(bondedAmt),
			func() {},
			sdk.ZeroDec(),
		},
	}
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.ctx = suite.ctx.WithChainID("hekas_9899-1")
			tc.malleate()
			...
			teamAlloc := sdk.NewDecCoin(
				types.DefaultInflationDenom,
				sdk.TokensFromConsensusPower(int64(200_000_000), hekastypes.PowerReduction),
			)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.isMainnet {
				...
				suite.ctx = suite.ctx.WithChainID("hekas_9898-666")
			}
			tc.malleate()
