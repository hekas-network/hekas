...
import (
	...
	"github.com/hekas-network/hekas/server/config"
	...
	"github.com/hekas-network/hekas/x/evm/statedb"
...
func (suite *KeeperTestSuite) DoSetupTest(t require.TestingT) {
	...
	header := testutil.NewHeader(
		1, time.Now().UTC(), "hekas_9899-1", consAddress, nil, nil,
	)
...
func (suite *KeeperTestSuite) SetupIBCTest() {
	...
	s.app = suite.HekasChain.App.(*app.Hekas)
	evmParams := s.app.EvmKeeper.GetParams(s.HekasChain.GetContext())
	evmParams.EvmDenom = utils.BaseDenom
	err := s.app.EvmKeeper.SetParams(s.HekasChain.GetContext(), evmParams)
	suite.Require().NoError(err)

	// s.app.FeeMarketKeeper.SetBaseFee(s.HekasChain.GetContext(), big.NewInt(1))
	...
	validators := s.app.StakingKeeper.GetValidators(suite.HekasChain.GetContext(), 2)
	cons, err := validators[0].GetConsAddr()
	suite.Require().NoError(err)
	suite.HekasChain.CurrentHeader.ProposerAddress = cons.Bytes()

	err = s.app.StakingKeeper.SetValidatorByConsAddr(suite.HekasChain.GetContext(), validators[0])
	suite.Require().NoError(err)

	_, err = s.app.EvmKeeper.GetCoinbaseAddress(suite.HekasChain.GetContext(), sdk.ConsAddress(suite.HekasChain.CurrentHeader.ProposerAddress))
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(suite.HekasChain.GetContext(), inflationtypes.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(suite.HekasChain.GetContext(), inflationtypes.ModuleName, suite.HekasChain.SenderAccount.GetAddress(), coins)
	suite.Require().NoError(err)
	...
	err = s.app.BankKeeper.MintCoins(s.HekasChain.GetContext(), types.ModuleName, coins)
	...
	err = s.app.ClaimsKeeper.SetParams(suite.HekasChain.GetContext(), claimparams)
	suite.Require().NoError(err)

	params := types.DefaultParams()
	params.EnableErc20 = true
	err = s.app.Erc20Keeper.SetParams(suite.HekasChain.GetContext(), params)
	suite.Require().NoError(err)
	...
	coinHekas = sdk.NewCoin(utils.BaseDenom, sdk.NewInt(1000000000000000000))
	coins = sdk.NewCoins(coinHekas)
	err = s.app.BankKeeper.MintCoins(suite.HekasChain.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(suite.HekasChain.GetContext(), types.ModuleName, authtypes.FeeCollectorName, coins)
	suite.Require().NoError(err)
}
...
// to the Hekas chain (used on IBC tests)
func (suite *KeeperTestSuite) DeployContractToChain(name, symbol string, decimals uint8) (common.Address, error) {
	return testutil.DeployContract(
		s.HekasChain.GetContext(),
		s.HekasChain.App.(*app.Hekas),
		suite.HekasChain.SenderPrivKey,
		suite.queryClientEvm,
		contracts.ERC20MinterBurnerDecimalsContract,
		name, symbol, decimals,
	)
...
// Send back coins (from path endpoint B to A). In case of IBC coins need to provide ibcCoinMetadata (<port>/<channel>/<denom>, e.g.: "transfer/channel-0/ahekas") as input parameter.
