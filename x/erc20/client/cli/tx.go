...
func NewConvertCoinCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 2 {
				receiver = args[1]
				if err := hekastypes.ValidateAddress(receiver); err != nil {
...
func NewConvertERC20Cmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contract := args[0]
			if err := hekastypes.ValidateAddress(contract); err != nil {
...

{
  "metadata": [
	...
	cmd.Flags().String(cli.FlagDeposit, "1ahekas", "deposit of proposal")
