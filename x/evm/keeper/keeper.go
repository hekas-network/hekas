...
func (k *Keeper) WithChainID(ctx sdk.Context) {
	chainID, err := hekastypes.ParseChainID(ctx.ChainID())
	if err != nil {
		panic(err)
	}
	...
	if !(chainID.Cmp(big.NewInt(9899)) == 0 || chainID.Cmp(big.NewInt(9898)) == 0) {
		panic("EVM only supports Hekas chain identifiers (9898 or 9899)")
	}
...
func (k *Keeper) GetAccountWithoutBalance(ctx sdk.Context, addr common.Address) *statedb.Account {
	...
	ethAcct, ok := acct.(hekastypes.EthAccountI)
	if ok {
