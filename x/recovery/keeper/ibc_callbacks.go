...
import (
	...
	"github.com/hekas-network/hekas/ibc"
...
//   - sends over all Hekas native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `hekas1` and the original bech32 format
	...
	// a sender's own account on Hekas (sender == recipient)
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		packetTransfer := &transfertypes.MsgTransfer{
			...
			Sender:           recipient.String(),        // recipient is the address in the Hekas chain
...
// the IBC denomination, i.e port and channel on Hekas for the voucher. It
// returns an error if:
