...
func (k Keeper) MergeClaimsRecords(
	...
	for _, action := range actions {
		...
		//  - the sender is not an hekas address and can't claim vote, delegation or evm actions
		...
		if senderClaimsRecord.HasClaimedAction(action) {
			return types.ClaimsRecord{}, errorsmod.Wrapf(errortypes.ErrNotSupported, "non-hekas sender must not have claimed action: %v", action)
		}
