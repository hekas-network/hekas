...
func (suite *KeeperTestSuite) TestDelegationAutoWithdrawAndDelegateMore() {
	...
	validator, _ = validator.AddTokensFromDel(sdk.TokensFromConsensusPower(1, hekastypes.PowerReduction))
	delAmount := sdk.TokensFromConsensusPower(1, hekastypes.PowerReduction)
...
func (suite *KeeperTestSuite) TestClawbackEmptyAccountsAirdrop() {
	...
	tests := []struct {
		...
		{
			name:           "address active",
			address:        "hekas1qql8ag4cluz6r4dz28p3w00dnc9w8ueu3790nw",
			sequence:       1,
		...
		},
		{
			...
			address:        "hekas1x2w87cvt5mqjncav4lxy8yfreynn273xlr0cxj",
			sequence:       0,
			expectClawback: true,
			claimsRecord: types.ClaimsRecord{
				InitialClaimableAmount: sdk.NewInt(100),
