...
# Set client config
hekasd config keyring-backend "$KEYRING"
hekasd config chain-id "$CHAINID"

# if $KEY exists it should be deleted
hekasd keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Hekas (Moniker can be anything, chain-id must be an integer)
hekasd init "$MONIKER" --chain-id "$CHAINID"
...
jq '.app_state.evm.params.evm_denom="ahekas"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.inflation.params.mint_denom="ahekas"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
node_address=$(hekasd keys list | grep  "address: " | cut -c12-)
...
# 0xA61808Fe40fEb8B3433778BBC2ecECCAA47c8c47 || hekas15cvq3ljql6utxseh0zau9m8ve2j8erz8fv28yu
jq -r --arg amount_to_claim "$amount_to_claim" '.app_state.bank.balances += [{"address":"hekas15cvq3ljql6utxseh0zau9m8ve2j8erz8fv28yu","coins":[{"denom":"ahekas", "amount":$amount_to_claim}]}]' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

# disable produce empty block
sed -i 's/create_empty_blocks = true/create_empty_blocks = false/g' "$CONFIG_TOML"

# Allocate genesis accounts (cosmos formatted addresses)
hekasd add-genesis-account $KEY 100000000000000000000000000ahekas --keyring-backend $KEYRING
...
hekasd gentx $KEY 1000000000000000000000ahekas --keyring-backend $KEYRING --chain-id "$CHAINID"
## In case you want to create multiple validators at genesis
## 1. Back to `hekasd keys add` step, init more keys
## 2. Back to `hekasd add-genesis-account` step, add balance for those
## 3. Clone this ~/.Hekasd home directory into some others, let's say `~/.clonedHekasd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedHekasd/config/gentx/` folders into the original `~/.Hekasd/config/gentx`

# Collect genesis tx
hekasd collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
hekasd validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
hekasd start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001ahekas --json-rpc.api eth,txpool,personal,net,debug,web3
