...
// TestUpgrade tests if an Hekas node can be upgraded from one version to another.
...
func (s *IntegrationTestSuite) TestUpgrade() {
	if s.upgradeManager == nil {
		s.T().Log("no upgrade manager")
		return
	}
	for idx, version := range s.upgradeParams.Versions {
		if idx == 0 {
			// start initial node
			err := s.runInitialNode(version, registryDockerFile)
...
func (s *IntegrationTestSuite) TestCLITxs() {
	...
	err := s.runInitialNode(mainBranch, repoDockerFile)
	if err != nil {
		s.T().Log(err)
		return
	}
	...
	}{
		{
			...
			cmd: func() (string, error) {
				return s.upgradeManager.CreateSubmitProposalExec(
					...
					5000,
					true,
					"--fees=5000000000ahekas",
					"--gas-prices=50000ahekas",
				)
			},
			expPass:   false,
