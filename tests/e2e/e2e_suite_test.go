...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	switch _, err := os.Stat(upgradePath); {
...
// runInitialNode builds a docker image capable of running an Hekas node with the given version.
// After a successful build, it runs the container and checks if the node can produce blocks.
func (s *IntegrationTestSuite) runInitialNode(version upgrade.VersionConfig, dockerFile string) error {
	if s.upgradeManager == nil {
		return errors.New("nil upgrade manager")
	}
	...
	if err != nil {
		errorsmod.Wrapf(err, "can't build container with Hekas version: %s", version)
		return err
	}
	...
	if err != nil {
		errorsmod.Wrapf(err, "can't run node with Hekas version: %s", version)
		return err
	}
...
func (s *IntegrationTestSuite) proposeUpgrade(name, target string) {
	...
	// if Hekas is lower than v10.x.x no need to use the legacy proposal
	currentVersion, err := s.upgradeManager.GetNodeVersion(ctx)
	s.Require().NoError(err, "can't get current Hekas version")
	...
	s.Require().NoErrorf(
		err,
		"can't create the proposal to upgrade Hekas to %s at height %d with name %s",
	...
	s.Require().NoErrorf(
		err,
		"failed to submit proposal to upgrade Hekas to %s at height %d\nstdout: %s,\nstderr: %s",
...
func (s *IntegrationTestSuite) voteForProposal(id int) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	exec, err := s.upgradeManager.CreateVoteProposalExec(s.upgradeParams.ChainID, id, "--fees=500ahekas", "--gas=500000")
...
func (s *IntegrationTestSuite) upgrade(targetRepo, targetVersion string) {
	...
	// export node .hekasd to local build/
	...
	node.SetCmd([]string{"hekasd", "start", fmt.Sprintf("--chain-id=%s", s.upgradeParams.ChainID)})
	err = s.upgradeManager.RunNode(node)
