...
func (m *Manager) BuildImage(name, version, dockerFile, contextDir string, args map[string]string) error {
	...
	opts := docker.BuildImageOptions{
		...
		BuildArgs:  buildArgs,
		// name with tag, e.g. hekas:v9.0.0
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start hekas node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Hekas node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Hekas CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	exec, err := m.CreateExec([]string{"hekasd", "q", "block"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("hekas query error: %s", errBuff.String())
	}
	return h, nil
}

// GetNodeVersion calls the Hekas CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"hekasd", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("hekas version error: %s", errBuff.String())
	}
