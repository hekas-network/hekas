...
// HekasVersions is a custom comparator for sorting semver version strings.
type HekasVersions []string

// Len is the number of stored versions..
func (v HekasVersions) Len() int { return len(v) }

// Swap swaps the elements with indexes i and j. It is needed to sort the slice.
func (v HekasVersions) Swap(i, j int) { v[i], v[j] = v[j], v[i] }

// Less compares semver versions strings properly
func (v HekasVersions) Less(i, j int) bool {
...
func CheckLegacyProposal(version string) bool {
	...
	cmp := HekasVersions([]string{version, "v10.0.0"})
...
func RetrieveUpgradesList(upgradesPath string) ([]string, error) {
	...
	sort.Sort(HekasVersions(versions))

	return versions, nil
}

// ExportState executes the  'docker cp' command to copy container .hekasd dir
...
func (m *Manager) ExportState(targetDir string) error {
	...
	cmd := exec.Command(
		"docker",
		"cp",
		fmt.Sprintf("%s:/root/.hekasd", m.ContainerID()),
		targetDir,
	)
	return cmd.Run()
}
