...
func DeployContract(
	...
	chainID := hekasApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := hekasApp.EvmKeeper.GetNonce(ctx, from)
	...
	msgEthereumTx := evm.NewTx(&evm.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: hekasApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := DeliverEthTx(hekasApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, hekasApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := hekasApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := hekasApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := hekasApp.EvmKeeper.GetNonce(ctx, from)
