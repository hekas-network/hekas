...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("hekas_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
		BondDenom:         "ahekas",
		MinGasPrices:      fmt.Sprintf("0.000006%s", hekastypes.AttoHekas),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, hekastypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, hekastypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, hekastypes.PowerReduction),
...
// NewAppConstructor returns a new Hekas AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !hekastypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "hekasd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "hekascli")
		...
		customAppTemplate, _ := config.AppConfig(hekastypes.AttoHekas)
