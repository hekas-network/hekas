...
import (
	...
	"github.com/hekas-network/hekas/testutil/tx"
)
...
//  3. EndBlock
//  4. Commit
func Commit(ctx sdk.Context, app *app.Hekas, t time.Duration, vs *tmtypes.ValidatorSet) (sdk.Context, error) {
...
func DeliverTx(
	...
	return BroadcastTxBytes(appHekas, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	tx, err := tx.PrepareEthTx(txConfig, appHekas, priv, msgs...)
	if err != nil {
		return abci.ResponseDeliverTx{}, err
	}
	return BroadcastTxBytes(appHekas, txConfig.TxEncoder(), tx)
...
func CheckTx(
	...
	return checkTxBytes(appHekas, txConfig.TxEncoder(), tx)
}
...
func BroadcastTxBytes(app *app.Hekas, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseDeliverTx, error) {
...
func checkTxBytes(app *app.Hekas, txEncoder sdk.TxEncoder, tx sdk.Tx) (abci.ResponseCheckTx, error) {
	bz, err := txEncoder(tx)
	if err != nil {
		return abci.ResponseCheckTx{}, err
	}
